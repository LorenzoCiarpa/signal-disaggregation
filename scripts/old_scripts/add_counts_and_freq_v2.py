...
def main():
    infile=Path('responses_per_imei_clean.xlsx')
    survey=Path('NILM Questionario (Risposte).xlsx')
    outfile=Path('responses_per_imei_clean_v2.xlsx')
    ...
    wb = pd.read_excel(infile, sheet_name=None)
    updated = {}
    changes = []
        ...
        for idx, row in df.iterrows():
            ...
            if cnt_col is not None:
                ...
                if pd.isna(raw):
                    ...
                    sval = str(raw).strip()
                    if 'non presente' in sval.lower() or sval.lower()=='no':
                        cnt_val = 0
                    else:
