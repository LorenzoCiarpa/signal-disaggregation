"""
Day-wise Gurobi NILM with soft time-window penalties (constrained_v4).

Identical pipeline to approach_gurobi_15min but uses constrained_v4:
time-window constraints are applied as objective penalties instead of
hard UB=0 exclusions, allowing the solver to occasionally activate a
device outside its window when the reconstruction benefit is large enough.
"""
...
from scripts.nilm.gurobi_methods import constrained_v4
...
def run(
    ...
    """Disaggregate day by day with soft time-window penalties.
    ...
    for day_idx, day in enumerate(unique_days):
        ...
        day_result, info = constrained_v4(
            signal=day_signal,
