"""
Day-wise Gurobi NILM with the survey's timing priors fully applied.

Builds on approach_gurobi_weekly_quota and closes two gaps in how the questionnaire
was being used.
...
Window specificity
------------------
A device that declared no time window used to pay nothing, anywhere: it was a free
wildcard that could absorb any plateau at any hour, while a device sitting inside its
own declared window also paid nothing.  They tied, and the choice between them was
arbitrary.

The fix is not to punish devices for having no schedule — many genuinely have none,
and that is absence of information, not evidence of implausibility.  Instead "no
window" is read as "window = 24h" and the penalty scales with how little the
declaration narrows things down (see time_windows.window_vagueness).  A dishwasher
that named 19:00-22:00 scores 0.35 against an air conditioner's 1.00, so inside that
evening window the dishwasher wins the tie — while at 10:00 the ranking correctly
reverses, because the dishwasher is then far outside what it declared.
...
def run(
    ...
    window_vagueness_weight: float = 0.2,
    season_penalty: float = 8.0,
    verbose: bool = False,
) -> dict[str, pd.Series]:
    ...
    Args:
        ...
        window_vagueness_weight: Weight on the vagueness floor, which is what lets a
            narrowly-declared device beat an undeclared one inside its window.  Tuned
            on real homes: 0.2 shifts attribution toward declared devices for a few
            percent of reconstruction error, while 1.0 drops whole devices and
            degrades the fit badly.
        season_penalty: Cost per out-of-season ON slot.  The default is high enough to
            remove a device outside its declared months in practice.
    ...
    active_months: dict[str, list[int]] = {}
    ...
    for dev in event_devices:
        ...
        if dev.active_months:
            active_months[dev.name] = list(dev.active_months)
    ...
    for day in unique_days:
        ...
        day_result, info = solve_full(
            ...
            window_vagueness_weight=window_vagueness_weight,
            season_penalty=season_penalty,
            active_months_by_device=active_months or None,
        )
