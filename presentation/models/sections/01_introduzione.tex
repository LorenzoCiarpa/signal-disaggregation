...
I modelli sono presentati in ordine di costruzione: ciascuno estende il precedente aggiungendo
un elemento, e l'ultimo (\code{cvxpy\_survey\_prior}) è quello attualmente in uso. Per ogni
modello vengono date le variabili di decisione, la funzione obiettivo e l'insieme completo dei
vincoli.
...
Le chiavi degli approcci nel codice iniziano con il prefisso \code{cvxpy\_} per ragioni
storiche, ma \textbf{CVXPY non è più utilizzato}. I modelli sono costruiti e risolti invocando
direttamente HiGHS attraverso la libreria \code{highspy}. Il prefisso è stato mantenuto per non
rompere la continuità con i risultati già prodotti.
\end{notebox}
